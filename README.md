...
`convo_cli.py` lets you chat with PolicyBot from the terminal and automatically exports the session as a RAGAS / DeepEval-ready markdown file (`conversation.md`):
...
|------|---------|-------------|
| `--url` | `http://localhost:8001` | Backend base URL |
| `--timeout` | `60` | Request timeout in seconds |

Type `exit` or `0` at any prompt to end the session. On exit, `conversation.md` is written (and overwritten each run) with:

- **RAGAS** — `HumanMessage` / `AIMessage` conversation array, retrieved contexts (last turn), and ground truth (last turn)
- **DeepEval** — one `LLMTestCase` per turn with `input`, `actual_output`, `retrieval_context`, and `expected_output` (ground truth) populated for every turn
...
- [convo_cli.py](convo_cli.py) — terminal chat tool with RAGAS / DeepEval markdown export
