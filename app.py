...
def _inject_theme():
    ...
    <style>
        ...
        /* Main-area input labels — navy + bold.
           Scoped to the main section (NOT a :not(sidebar) trick, which leaks
           through the sidebar's inner wrapper divs and turns its white labels
...
def _sidebar() -> tuple[str, str, str, str, str]:
    with st.sidebar:
        ...
        if st.session_state.pop("cfg_saved_flash", False):
        ...
        if st.button("Save Configuration", use_container_width=True):
            ...
            if updated:
                ...
                # Re-seed the password field with the saved-key sentinel on the NEXT
                # run. We can't assign to st.session_state.ui_api_key here because that
                # widget is already instantiated this run (Streamlit forbids it). Drop
                # the key and rerun so the pre-widget seeding above repopulates it.
                st.session_state.pop("ui_api_key", None)
                st.session_state["cfg_saved_flash"] = True
                st.rerun()
            else:
