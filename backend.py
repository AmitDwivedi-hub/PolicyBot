...
GROUND_TRUTH_SYSTEM_PROMPT = (
    "You are a precise policy document analyst. "
    "Using only the provided policy context, produce the single most accurate and complete "
    "answer to the question. Include all relevant numbers, dates, percentages, and specific "
    "clause references. Answer in 1-3 sentences with no hedging or caveats."
)
...
def query(req: QueryRequest):
    ...
    hits = _engine.search(req.question, k=TOP_K)
    result["ground_truth"] = _generate_ground_truth(
        req.question, hits, provider, api_key, model, base_url, api_version
    )
    return result
