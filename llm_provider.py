...
def chat(
    ...
    if p == "anthropic":
        return _call_anthropic(api_key, system_prompt, messages, max_completion_tokens, model or ANTHROPIC_DEFAULT_MODEL)
    if not base_url:
...
def _call_openai(api_key: str, system_prompt: str, messages: list[dict], max_completion_tokens: int, model: str, base_url: str, api_version: str) -> str:
    try:
